...
async def follow_chat(links, app, timer, client_config, yaml_config, ini_config):
    ...
    for link in links:
        try:
            ...
            print_spammer_status(client_name, client_config["success_autojoiner_counter"], 25)
...
async def get_chat_members_by_chat_id(dialog, client, client_name, client_config):
    members_of_chat = []
    try:
        async for member in client.get_chat_members(chat_id=dialog.chat.id):
            if member.user.is_bot is not True:
                member_id = member.user.id
                members_of_chat.append(member_id)
    except (FloodWait, Flood, FloodTestPhoneWait) as e:
        print_spammer_status(client_name, client_config["success_teger_counter"], e.value)
        await asyncio.sleep(e.value)
    except:
        print_spammer_status(client_name, client_config["success_teger_counter"], 25)
        await asyncio.sleep(25)
    return members_of_chat


async def send_message(app: Client, dialog: Dialog, chunk: List, text: str):
    teger_text = ""
    for i in range(0, len(chunk)):
        teger_text += "[" + u'\u200c' + "]" + f"(tg://user?id={chunk[i]}) "
    teger_text += text
    await app.send_message(dialog.chat.id, teger_text)
...
async def teger_1(app, timer, count_chat_for_send, mode_edit, client_config,
        ...
        if count_chat_for_send is None:
            try:
                ...
                for dialog in dialogs:
                    members_of_chat = await get_chat_members_by_chat_id(dialog, app, client_name, client_config)
                    if len(members_of_chat) == 0:
                        continue
                    try:
                        text = [message async for message in app.get_chat_history("me", 1)][0].text
                        ...
                        await asyncio.sleep(25)
                        continue
                    for i in range(0, len(members_of_chat), 5):
                        inner_timer = timer
                        if type(timer) == list:
                            inner_timer = randint(timer[0], timer[1])
                        chunk = members_of_chat[i:i + 5]
                        try:
                            await send_message(app, dialog, chunk, text)
                            client_config["success_teger_counter"] += 1
                            ini_config["Settings"]["teger_counter"] = str(
                                int(ini_config["Settings"]["teger_counter"]) + 1)
                        except (FloodWait, Flood, FloodTestPhoneWait) as e:
                            print_spammer_status(client_name, client_config["success_teger_counter"], e.value)
                            await asyncio.sleep(e.value)
                        except:
                        ...
                        write_ini(ini_config)
                        write_yaml(CLIENTS_SESSIONS_INFO, yaml_config)
                        print_spammer_status(client_name, client_config["success_teger_counter"], inner_timer)
...
async def teger_2(app, timer, count_chat_for_send, mode_edit, client_config,
                  yaml_config, ini_config):
    counter = 0
    client_name = get_client_name(app)
    while True:
            ...
            try:
                ...
                for dialog in dialogs:
                    ...
                    for i in range(0, len(members_of_chat), 5):
                        ...
                        write_ini(ini_config)
                        write_yaml(CLIENTS_SESSIONS_INFO, yaml_config)
                        print_spammer_status(client_name, client_config["success_teger_counter"], 2)
...
async def main():
    ...
    if config_ini_path.is_file() is not True:
        ...
        config_ini["Settings"] = {"autojoiner_counter": 0, "teger_counter": 0}
    else:
    ...
    if command == 2:
        ...
        modes = [teger_1, teger_2]
        mode = int(input(
            "[1] - рассылка по 1 чату\n"
            "[2] - выбрать количество чатов для отправки\nВведите число, соответствующее выбранной опции: "
        ))
